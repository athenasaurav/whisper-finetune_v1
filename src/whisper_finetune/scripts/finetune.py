...
def main(config):
    ...
    # Pass language and eval vocab spec to training/eval (e.g. "ar" → VOCAB_SPECS["arabic"])
    config["training"]["dataset_language"] = ds_config.get("language", default_language)
    config["training"]["eval_vocab_spec"] = "arabic" if ds_config.get("language") == "ar" else "v0"

    # Get tokenizer (language for decoder lang token, e.g. "ar" for Arabic)
    ds_language = ds_config.get("language", default_language)
