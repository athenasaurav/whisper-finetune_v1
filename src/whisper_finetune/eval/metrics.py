...
class DatasetMetrics:
    ...
    # Optional inference latency (when measure_inference_latency is True)
    inference_ms_per_sample: Optional[float] = None
    inference_samples_per_sec: Optional[float] = None
...
def aggregate_dataset_metrics(per_utterance_metrics: List[PerUtteranceMetrics], dataset_name: str) -> DatasetMetrics:
    ...
    if len(per_utterance_metrics) == 0:
        return DatasetMetrics(
            ...
            inference_samples_per_sec=None,
        )
...
def compute_macro_average(dataset_metrics: List[DatasetMetrics]) -> Dict[str, float]:
    ...
    if len(dataset_metrics) == 0:
        return {
            ...
            "macro_ece": 0.0,
            "macro_ms_per_sample": None,
            "macro_samples_per_sec": None,
        }
    ...
    lat = [m.inference_ms_per_sample for m in dataset_metrics if m.inference_ms_per_sample is not None]
    if lat:
        out["macro_ms_per_sample"] = float(np.mean(lat))
        out["macro_samples_per_sec"] = float(1000.0 / out["macro_ms_per_sample"])
    else:
        out["macro_ms_per_sample"] = None
        out["macro_samples_per_sec"] = None
    return out
